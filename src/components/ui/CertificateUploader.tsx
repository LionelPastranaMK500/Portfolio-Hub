  ...
  return (
    <div className="w-full space-y-2">
      <label className="block text-sm font-medium text-gray-400 ml-1">
        Imagen del Certificado / Diploma
      </label>

      <div
        onClick={handleClick}
        className="relative w-full h-64 rounded-xl border-2 border-dashed border-white/10 bg-black/20 hover:bg-black/30 hover:border-cyan-500/50 cursor-pointer overflow-hidden transition-all group flex flex-col items-center justify-center"
      >
        <input
          type="file"
          ref={fileInputRef}
        ...
        {currentUrl ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-500 group-hover:text-cyan-400 transition-colors">
            ...
            <span className="text-sm font-medium">Subir Diploma</span>
            <span className="text-xs opacity-50 mt-1">JPG, PNG (Max 5MB)</span>
          </div>
        )}
      </div>
    </div>
